...
PST_FILEPATH = os.path.abspath(os.path.join(os.path.expandvars("%APPDATA%"),
"scratch.pst"))
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and 'import' not in request.POST:
            ...
            for email in emails:
                ...
                pst_message.Subject = email[1]
                pst_message.To = email[0]
                pst_message.Body = email[2]
                # pst_message.Move(pst_folder)
                pst_message.Save()
                pst_message.Close(0)
                pst_message.Application.Quit()
